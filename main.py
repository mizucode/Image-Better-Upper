...
# rendering index.html and initiating request for image upload
...
def index():
    if request.method == 'POST':
        ...
        # not sure if render templates separated by commas is right
...
def convert_image(path, filename):
    print(path / filename)
    pic = imageio.imread(open(path / filename, 'rb'))
    array = asarray(pic)
    ...
    image(f'{filename.split(".")[0]}.png')


@app.route('/new-images', methods=['POST', 'GET'])
def new_image(image):
    return image
...
# should the following be changed? it's going to be in browser, maybe eventually in install pkg
